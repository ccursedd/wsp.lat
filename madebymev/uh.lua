--[[
	compass.lol UI Library — Single-file ModuleScript.
	
	=== HOW TO USE ===
	
	1. Upload this file as a ModuleScript to Roblox (e.g. in ReplicatedStorage),
	   then:
	
	local Compass = require(path.to.Compass)
	
	-- OR load directly from GitHub with loadstring:
	
	local Compass = loadstring(game:HttpGet(
		"https://raw.githubusercontent.com/ccursedd/wsp.lat/refs/heads/main/madebymev/ui.lua"
	))()
	
	-- NOTE: "local Compass = 'https://...'" does NOT work!
	-- That just stores a URL string. You MUST use loadstring(game:HttpGet(...))()
	-- to download and run the Lua code.
	...
	Main:AddToggle({ Name = "Reanimation", Default = false, Callback = function(State) end })
	Main:AddButton({ Name = "Execute", Callback = function() end })
...
function Window:_buildContent()
	...
	self._scrollLayout = new("UIListLayout", {
		Padding = UDim.new(0, 5), SortOrder = Enum.SortOrder.LayoutOrder,
		Parent = self._scroll,
	})
	new("UIPadding", {
	...
	self._scrollLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
		self._scroll.CanvasSize = UDim2.fromOffset(0, self._scrollLayout.AbsoluteContentSize.Y + 10)
	end)
end
