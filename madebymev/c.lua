...
function WinMT.New(opt)
	...
	-- Viewport
	pcall(function() if opt.ShowViewport ~= false then self:_Viewport() end end)

	-- Snowfall
	pcall(function() if opt.Snowfall ~= false then self:_Snow() end end)
...
function WinMT:_Viewport()
	...
	Players.LocalPlayer.CharacterAppearanceLoaded:Connect(function(char)
		task.wait(0.2); pcall(rebuild); pcall(watchC, char)
	end)
	if Players.LocalPlayer.Character then
		task.wait(0.5)
		if not st.clone then pcall(loadC); if st.clone then pcall(watchC, Players.LocalPlayer.Character) end end
	end
	...
	vp.InputChanged:Connect(function(i)
		if st.drag and (i.UserInputType == Enum.UserInputType.MouseMovement or i.UserInputType == Enum.UserInputType.Touch) then
			...
			st.tph = math.max(st.phMin, math.min(st.phMax, st.tph + ((i.Position.Y - st.ly) * 0.01)))
...
function WinMT:_Snow()
	...
	local con = RunSvc.RenderStepped:Connect(function(dt)
		...
		for i = #flakes, 1, -1 do
			...
			if not d.obj.Parent then table.remove(flakes, i)
				...
				local x = math.max(d.sz, math.min(sz.X - d.sz, rx))
	...
	self._stopSnow = function()
		...
		for i = #flakes, 1, -1 do flakes[i] = nil end
	end
end
